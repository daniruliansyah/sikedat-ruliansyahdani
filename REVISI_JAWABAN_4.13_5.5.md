...
| `/aktivitas` | GET | Log aktivitas sistem (klasifikasi, validasi, *retrain*) | Admin, Analis, dan Staff |
...
Dari Tabel 4.31 terlihat bahwa sistem SIKEDAT memisahkan *endpoint* ke dalam tiga tingkatan akses. *Endpoint* publik (tanpa login) hanya mencakup beranda, prediksi, dan autentikasi — halaman yang memang dirancang untuk dapat diakses oleh masyarakat umum. *Endpoint* dengan *roles* Admin, Analis, dan Staff mencakup halaman-halaman informatif seperti *dashboard* dan log aktivitas yang membutuhkan konteks login namun tidak melibatkan pengelolaan data sensitif. *Endpoint* yang dibatasi untuk Admin dan Analis mencakup seluruh operasi yang mengubah data: klasifikasi, validasi, dan *retrain* — sesuai dengan tanggung jawab operasional kedua peran tersebut di lingkungan DISHUB Surabaya. Khusus untuk *endpoint* manajemen akun pengguna (`/users`), akses hanya diberikan kepada Admin karena menyangkut keamanan seluruh sistem.
...
Tabel di atas menunjukkan bahwa tabel `roles` dirancang minimalis dengan hanya empat kolom. Kolom `akses_penuh` bertipe *boolean* menjadi satu-satunya penentu hierarki akses dalam sistem — desain yang disengaja untuk menjaga kesederhanaan logika otorisasi tanpa perlu mendefinisikan banyak *permission flag*.
