...
Tabel 4.X berikut merangkum seluruh *endpoint* HTTP yang tersedia pada sistem SIKEDAT beserta fungsi dan tingkat aksesnya.

| Route | Metode | Fungsi | Tingkat Akses |
|---|---|---|---|
| `/` | GET | Beranda — statistik ringkas dan grafik komposisi kendaraan | Publik |
| `/prediksi` | GET, POST | Prediksi kepadatan berdasarkan input hari, jam, dan menit | Publik |
| `/auth/login` | GET, POST | Autentikasi pengguna dengan *username* dan kata sandi | Publik |
| `/auth/logout` | GET | Mengakhiri sesi pengguna yang aktif | Login |
| `/dashboard` | GET | *Dashboard* analitik: heatmap, grafik per jam, *weekday vs weekend* | Login |
| `/data-observasi` | GET | Tabel seluruh data observasi dengan filter status validasi dan paginasi | Login |
| `/aktivitas` | GET | Log aktivitas sistem (klasifikasi, validasi, *retrain*) | Login |
| `/classifier` | GET, POST | Halaman unggah file CSV untuk diklasifikasikan | Akses Penuh |
| `/classifier/run` | POST | Menjalankan proses klasifikasi *batch* | Akses Penuh |
| `/classifier/download` | GET | Mengunduh hasil klasifikasi sebagai file CSV | Akses Penuh |
| `/validasi` | GET | Antarmuka validasi dan koreksi label per *batch* | Akses Penuh |
| `/validasi/save` | POST | Menyimpan hasil validasi ke basis data | Akses Penuh |
| `/validasi/import` | POST | Mengimpor hasil klasifikasi ke antrian validasi | Akses Penuh |
| `/retrain` | GET, POST | Halaman manajemen dan pemantauan *retrain* model | Akses Penuh |
| `/retrain/activate/<version>` | POST | Mengaktifkan versi model tertentu dari riwayat | Akses Penuh |
...
Tabel 4.X berikut merangkum seluruh teknologi yang digunakan dalam pengembangan sistem web SIKEDAT.
...
Gambar 4.18 menampilkan halaman autentikasi SIKEDAT yang dapat diakses melalui alamat `/auth/login`. Halaman ini menyajikan formulir *login* yang meminta *username* dan kata sandi, dengan tombol "Login Dishub" sebagai pemicu verifikasi identitas. Sistem menggunakan Flask-Login untuk manajemen sesi aktif dan Flask-Bcrypt untuk memverifikasi kata sandi yang tersimpan dalam bentuk *hash* bcrypt di basis data. Terdapat dua tingkat hak akses yang dibedakan berdasarkan nilai `akses_penuh` pada tabel `roles`: pengguna dengan akses penuh (Analis dan Admin Dishub) dapat menggunakan seluruh fitur termasuk klasifikasi, validasi, dan *retrain*, sedangkan pengguna *login* biasa (Staff) dapat mengakses *dashboard* analitik dan halaman informasi.
...
Tabel `roles` menyimpan definisi peran yang tersedia dalam sistem. Kolom `akses_penuh` bertipe *boolean* menjadi penentu apakah pengguna dengan peran tersebut dapat mengakses halaman-halaman yang memerlukan hak operasional penuh, yaitu klasifikasi data, validasi, dan *retrain* model.
...
| `akses_penuh` | BOOLEAN | `True` = dapat mengakses halaman *restricted* |
