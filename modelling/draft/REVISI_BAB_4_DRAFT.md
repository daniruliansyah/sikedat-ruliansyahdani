...
> **Status Draft (Revisi Struktur — sesuai urutan sub-bab Bab III, 3.3.1–3.3.13):**
> - Sub-bab 4.1 Studi Literatur → Baru ditambahkan sesuai struktur Bab 3
> - Sub-bab 4.2 Pengumpulan Data → Dipindah dari 4.1 lama
> - Sub-bab 4.3 Ekstraksi Data → Dipindah dari 4.2 lama
> - Sub-bab 4.4 Evaluasi Hasil Ekstraksi (MAPE) → Dipindah dari 4.3 lama
> - Sub-bab 4.5 Labeling Data → Dipindah dari 4.4 lama
> - Sub-bab 4.6 Preprocessing Data → Dipindah dari 4.5 lama
> - Sub-bab 4.7 Pembagian Data → Dipindah dari 4.6 lama
> - Sub-bab 4.8 Modelling → Dipindah dari 4.7 lama
> - Sub-bab 4.9 Hyperparameter Tuning → Digabung dari 4.7 lama (sesuai urutan Bab 3)
> - Sub-bab 4.10 Evaluasi Model → Dipindah dari 4.8 lama
> - Sub-bab 4.11 Pemilihan Model → Dipisah dari 4.8.3 lama
> - Sub-bab 4.12 Perancangan Sistem → `[TODO]` (sesuai 3.3.11)
> - Sub-bab 4.13 Pengembangan Web → `[TODO]` (sesuai 3.3.12)
> - Sub-bab 4.14 Evaluasi Sistem (SUS) → `[TODO]` (sesuai 3.3.13)

Pada bab ini disajikan hasil implementasi penelitian sesuai dengan tahapan metode penelitian yang telah dijabarkan pada Bab III. Hasil setiap tahap dipresentasikan dalam bentuk teks, tabel, dan gambar untuk memberikan gambaran yang utuh mengenai proses dan luaran penelitian, mulai dari studi literatur hingga evaluasi sistem aplikasi web berbasis Flask.

---

## 4.1  Hasil Studi Literatur

Studi literatur dalam penelitian ini bertujuan untuk mendalami metode computer vision menggunakan algoritma YOLOv8 untuk deteksi dan klasifikasi kendaraan, serta algoritma machine learning Random Forest dan Support Vector Machine untuk klasifikasi tingkat kepadatan lalu lintas. Kajian literatur mencakup penelusuran teori dasar tentang kepadatan lalu lintas dan indikator teknisnya, penelitian terdahulu yang menggunakan pendekatan serupa, serta best practices dalam implementasi sistem klasifikasi berbasis data visual dan numerik.

Referensi utama yang dikaji meliputi jurnal ilmiah, buku teks, dan dokumentasi teknis yang relevan dengan topik penelitian. Kajian ini kemudian menjadi landasan teoretis dalam perancangan sistem klasifikasi kepadatan lalu lintas pada penelitian ini, mencakup:

1. Teori deteksi objek berbasis deep learning, khususnya arsitektur YOLOv8 dan mekanisme tracking DeepSORT.
2. Konsep *cycle failure* (Nq1) dari Pedoman Kapasitas Jalan Indonesia (PKJI) 2023 dan *Highway Capacity Manual* (HCM) sebagai acuan labeling data.
3. Algoritma ensemble learning Random Forest dan Support Vector Machine beserta teknik hyperparameter tuning menggunakan GridSearchCV.
4. Metode evaluasi model machine learning termasuk *confusion matrix*, akurasi, presisi, recall, dan F1-score.
5. Kerangka pengembangan aplikasi web berbasis Flask serta metode evaluasi usabilitas menggunakan System Usability Scale (SUS).
...
Setelah proses ekstraksi data menggunakan algoritma YOLOv8 yang dijelaskan pada sub-bab 4.3, data ekstraksi disimpan dalam format Comma-Separated Values (CSV) dengan agregasi per interval 10 menit, sehingga setiap satu jam rekaman menghasilkan enam baris data. Total data tabular yang dihasilkan dari seluruh proses ekstraksi adalah **877 baris**, yang siap diolah pada tahap labeling dan preprocessing.

---

## 4.3  Hasil Ekstraksi Data dengan YOLOv8
...
### 4.3.1  Training Model YOLOv8 Custom
...
### 4.3.2  Hasil Deteksi dan Tracking
...
Dari proses ekstraksi keseluruhan dataset, diperoleh **877 baris data** yang masing-masing mewakili interval pengamatan 10 menit dengan struktur kolom: `Hari`, `Jam`, `Menit`, `Motor`, `Mobil`, `Bus`, `Truk`, dan `Total_Kendaraan`. Karakteristik lengkap distribusi data ini akan dianalisis lebih lanjut pada sub-bab 4.6.

---

## 4.4  Hasil Evaluasi Ekstraksi (MAPE)
...
4. Nilai MAPE dihitung menggunakan persamaan `[TODO: nomor persamaan MAPE di Bab II, misalnya (2.19)]` yang telah dijelaskan pada Bab II.

> **`[TODO: Buat Tabel 4.8 Hasil Perhitungan MAPE per Sampel]`
> Disarankan dengan kolom: `No | Hari | Jam | Menit | Kondisi Pencahayaan | Kategori | Hitung Manual (Ground Truth) | Hitung YOLO | APE (%)`. Setiap sampel diisi per kategori kendaraan (Motor, Mobil, Bus, Truk).

> **`[TODO: Buat Tabel 4.9 Ringkasan Rata-rata MAPE per Kategori Kendaraan]`**
> Kolom: `Kategori | Rata-rata APE (%) | Klasifikasi Lewis (1982)`. Isi baris: Motor, Mobil, Bus, Truk, dan Total (rata-rata keseluruhan).

`[TODO: Setelah kedua tabel di atas siap, tambahkan paragraf interpretasi dengan format seperti berikut:]`**
...
Pada dataset hasil ekstraksi, kolom `Tingkat_Kepadatan` awalnya berisi nilai frekuensi *cycle failure* (Nq1) dengan rentang 0 hingga 10. Sebanyak 463 baris atau 52,79% dari total data tercatat memiliki nilai Nq1 = 0 (tidak terjadi *cycle failure* sama sekali), sedangkan baris dengan Nq1 ≥ 3 sebanyak 209 baris atau 23,83%. Distribusi lengkap nilai Nq1 sebelum dikonversi ke kelas disajikan pada sub-bab 4.6 (EDA).

### 4.5.2  Konversi Frekuensi ke 3 Kelas Tingkat Kepadatan

Karena tujuan klasifikasi penelitian ini adalah membedakan tiga kelas kepadatan, yaitu Rendah, Sedang, dan Tinggi, nilai frekuensi *cycle failure* dikonversi menggunakan skema *threshold* yang disajikan pada Tabel 4.10.

**Tabel 4.10  Skema Konversi Nq1 ke Kelas Tingkat Kepadatan**
...
### 4.5.3  Pengembangan Dua Skenario Dataset
...
Distribusi akhir kelas pada kedua skenario disajikan pada Tabel 4.11 dan diilustrasikan pada Gambar 4.2.

**Tabel 4.11  Distribusi Kelas pada Kedua Skenario Dataset**
...
**Tabel 4.12  Hasil Deteksi Outlier dengan Metode IQR**
...
Penerapan One-Hot Encoding pada kolom `Hari` menghasilkan **7 kolom baru**, yaitu `Hari_Senin`, `Hari_Selasa`, `Hari_Rabu`, `Hari_Kamis`, `Hari_Jumat`, `Hari_Sabtu`, dan `Hari_Minggu`. Tabel 4.13 memberikan ilustrasi transformasi data sebelum dan sesudah One-Hot Encoding.

**Tabel 4.13  Ilustrasi Transformasi `Hari` dengan One-Hot Encoding**
...
### 4.6.5  Hasil Eksplorasi Data (EDA)

Sebelum masuk ke tahap pemodelan, dilakukan eksplorasi data (*Exploratory Data Analysis*) untuk memahami karakteristik dataset. Statistik deskriptif fitur volume kendaraan disajikan pada Tabel 4.14.

**Tabel 4.14  Statistik Deskriptif Fitur Volume Kendaraan**
...
Gambar 4.5 memperlihatkan distribusi yang juga merata antar jam pengamatan (106–114 baris per jam), yang mencerminkan konsistensi pengambilan data di setiap sesi. Terlihat dua kelompok jam yang terpisah: sesi pagi (jam 6–8) dan sesi sore-malam (jam 15–19), sesuai dengan rancangan pengumpulan data pada sub-bab 4.2. Tidak ada jam yang terlalu mendominasi, sehingga model mendapatkan representasi yang proporsional untuk seluruh rentang waktu pengamatan.
...
## 4.7  Hasil Pembagian Data (Train-Test Split)
...
Pembagian data dilakukan secara terpisah untuk masing-masing skenario, dengan hasil yang disajikan pada Tabel 4.15.

**Tabel 4.15  Hasil Pembagian Data Train-Test untuk Kedua Skenario**
...
Random Forest diimplementasikan menggunakan class `RandomForestClassifier` dari `scikit-learn`. Pada penelitian ini digunakan versi *hyperparameter grid* yang telah direvisi dari Tabel 3.11 pada Bab III, dengan pertimbangan efisiensi waktu komputasi tanpa mengurangi cakupan eksplorasi parameter utama (Opsi B-Slim). Detail *hyperparameter grid* dijabarkan pada sub-bab 4.9.

### 4.8.2  Support Vector Machine

SVM diimplementasikan menggunakan class `SVC` dari `scikit-learn`. Karena SVM sangat sensitif terhadap skala fitur, model diintegrasikan ke dalam *Pipeline* bersama `StandardScaler` untuk mencegah *data leakage* selama proses cross-validation. Detail *hyperparameter grid* dijabarkan pada sub-bab 4.9.

---

## 4.9  Hasil Hyperparameter Tuning

### 4.9.1  Random Forest

*Hyperparameter grid* yang diuji pada Random Forest disajikan pada Tabel 4.16, merupakan versi yang telah direvisi dari Tabel 3.11 pada Bab III.

**Tabel 4.16  Hyperparameter Grid untuk Random Forest**
...
Dari Tabel 4.16, pemilihan nilai-nilai parameter tersebut didasarkan pada pertimbangan berikut: `n_estimators` 100 dan 200 dipilih karena pada dataset dengan jumlah baris di bawah 1.000, penambahan pohon di atas 200 umumnya tidak memberikan peningkatan performa yang signifikan. `max_depth` None (tidak dibatasi) dan 10 dipilih untuk membandingkan model yang cenderung *overfit* vs model yang lebih terregularisasi. Nilai `min_samples_split` dan `min_samples_leaf` yang lebih kecil cenderung menghasilkan pohon yang lebih dalam dan kompleks, sementara nilai lebih besar berfungsi sebagai regularisasi.
...
Hasil GridSearchCV untuk Random Forest pada Skenario 1 ditampilkan pada Tabel 4.17 (16 kombinasi, diurutkan berdasarkan ranking F1-weighted).

**Tabel 4.17  Hasil Hyperparameter Tuning RF Skenario 1 (16 Kombinasi)**
...
Dari Tabel 4.17 terlihat bahwa kombinasi dengan `max_depth=10` (model terbatas) konsisten mendominasi peringkat teratas, menunjukkan bahwa pembatasan kedalaman pohon justru menghasilkan generalisasi yang lebih baik pada dataset ini dibandingkan pohon tanpa batas (`max_depth=None`). Kombinasi dengan `min_samples_leaf=1` juga mendominasi posisi atas, mengindikasikan bahwa granularitas klasifikasi yang lebih tinggi pada *leaf node* lebih menguntungkan untuk dataset ini.

Dari hasil GridSearchCV pada Tabel 4.17, kombinasi parameter terbaik untuk Random Forest pada Skenario 1 adalah:
...
dengan CV F1-weighted sebesar **0,7166**. Model RF Skenario 1 dengan kombinasi parameter terbaik ini selanjutnya dilatih ulang pada keseluruhan *training set* dan disimpan dalam format pickle (`model.pkl`) untuk digunakan pada tahap evaluasi (sub-bab 4.10).

#### Hasil Hyperparameter Tuning RF — Skenario 2

Hasil GridSearchCV untuk Random Forest pada Skenario 2 ditampilkan pada Tabel 4.18.

**Tabel 4.18  Hasil Hyperparameter Tuning RF Skenario 2 (16 Kombinasi)**
...
Kombinasi parameter terbaik untuk Random Forest Skenario 2 adalah:
...
*Hyperparameter grid* yang diuji pada SVM disajikan pada Tabel 4.19.

**Tabel 4.19  Hyperparameter Grid untuk SVM**
...
**Tabel 4.20  Hasil Hyperparameter Tuning SVM Skenario 1 (18 Kombinasi)**

| Rank | kernel | C | gamma | CV Accuracy | CV Precision | CV Recall | CV F1-weighted |
|---|---|---|---|---|---|---|---|
| 1 | rbf | 10,0 | 0,1 | 0,6534 | 0,6657 | 0,6534 | 0,6539 |
| 2 | rbf | 1,0 | 0,1 | 0,6420 | 0,6579 | 0,6420 | 0,6432 |
| 3 | rbf | 10,0 | scale | 0,6391 | 0,6565 | 0,6391 | 0,6421 |
| 4 | rbf | 1,0 | scale | 0,6420 | 0,6538 | 0,6420 | 0,6418 |
| 5 | rbf | 0,1 | 0,1 | 0,6149 | 0,6349 | 0,6149 | 0,6104 |
| 6 | poly | 1,0 | 0,1 | 0,6277 | 0,6355 | 0,6277 | 0,6091 |
...
**Tabel 4.21  Hasil Hyperparameter Tuning SVM Skenario 2 (18 Kombinasi)**
...
Pola serupa dengan Skenario 1 di mana kernel RBF mendominasi. Perbedaan utama adalah nilai `C` terbaik menurun dari 10 (S1) menjadi 1 (S2), yang mengindikasikan bahwa dengan distribusi kelas yang lebih seimbang pada Skenario 2, model SVM yang lebih *soft* (toleran terhadap kesalahan) justru lebih generalisatif.
...
### Ringkasan Best Model dari Tahap Hyperparameter Tuning

Hasil ringkasan empat *best model* dari tahap hyperparameter tuning (sebelum evaluasi pada *testing set*) disajikan pada Tabel 4.22.

**Tabel 4.22  Ringkasan Best Model Hasil Hyperparameter Tuning**
...
Dari Tabel 4.22 terlihat bahwa secara keseluruhan Random Forest menghasilkan CV F1-weighted lebih tinggi dibandingkan SVM pada masing-masing skenario yang sama, dengan selisih sekitar 6–5 poin persentase. Keempat *best model* di atas selanjutnya dievaluasi pada *testing set* untuk memperoleh gambaran performa model pada data yang belum pernah dilihat selama proses training, sebagaimana disajikan pada sub-bab 4.10.

---

## 4.10  Hasil Evaluasi Model

Pada sub-bab ini disajikan hasil evaluasi keempat *best model* yang telah diperoleh dari proses hyperparameter tuning pada sub-bab 4.9. Evaluasi dilakukan pada *testing set* (176 baris atau 20% dari dataset) yang belum pernah dilihat selama proses training. Hal ini bertujuan memperoleh gambaran objektif mengenai kemampuan generalisasi model pada data baru.
...
### 4.10.1  Evaluasi Random Forest

#### Evaluasi RF — Skenario 1

*Confusion matrix* hasil prediksi Random Forest pada *testing set* Skenario 1 disajikan pada Tabel 4.23.

**Tabel 4.23  Confusion Matrix Random Forest Skenario 1**
...
Dari Tabel 4.24, nilai *accuracy* keseluruhan sebesar 73,30% menunjukkan bahwa model berhasil mengklasifikasikan 129 dari 176 baris data *testing* dengan benar. Perbedaan yang mencolok antara F1-score kelas Rendah (0,8306), Tinggi (0,7312), dan Sedang (0,5000) mengindikasikan bahwa model lebih andal dalam membedakan kondisi ekstrem dibandingkan kondisi transisi.
...
**Tabel 4.25  Confusion Matrix Random Forest Skenario 2**
...
Dibandingkan Skenario 1, Gambar 4.9 menunjukkan peningkatan kemampuan model dalam mendeteksi kelas Sedang (recall meningkat dari 46,34% menjadi 61,64%), namun dengan trade-off penurunan recall pada kelas Rendah (81,72% menjadi 65,57%) dan Tinggi (80,95% menjadi 64,29%).

**Tabel 4.26  Metrik Evaluasi per Kelas — RF Skenario 2**
...
### 4.10.2  Evaluasi Support Vector Machine

#### Evaluasi SVM — Skenario 1

**Tabel 4.27  Confusion Matrix SVM Skenario 1**
...
**Tabel 4.28  Metrik Evaluasi per Kelas — SVM Skenario 1**
...
Karena SVM dengan kernel RBF tidak memiliki *feature importance* native, peneliti menggunakan **Permutation Importance** untuk memperoleh estimasi kontribusi fitur.
...
**Tabel 4.29  Confusion Matrix SVM Skenario 2**
...
**Tabel 4.30  Metrik Evaluasi per Kelas — SVM Skenario 2**
...
### 4.10.3  Komparasi Keempat Model

Ringkasan metrik evaluasi seluruh empat model pada *testing set* disajikan pada Tabel 4.31.

**Tabel 4.31  Ringkasan Komparasi Metrik Evaluasi 4 Model pada Testing Set**
...
Dari Gambar 4.15 terlihat secara visual bahwa RF Skenario 1 (batang paling kiri) secara konsisten memiliki nilai tertinggi pada seluruh lima metrik yang ditampilkan.
...
## 4.11  Hasil Pemilihan Model (Model Selection)

Berdasarkan hasil komparasi pada Tabel 4.31 dan visualisasi pada Gambar 4.15–4.17, **Random Forest Skenario 1** terpilih sebagai **model terbaik** dalam penelitian ini, dengan capaian sebagai berikut:
...
Pemilihan model terbaik didasarkan pada nilai F1-score weighted yang tertinggi di antara keempat model (0,7299), sebagai indikator utama yang mempertimbangkan distribusi kelas yang tidak seimbang secara proporsional. RF Skenario 1 unggul di seluruh lima metrik evaluasi utama sekaligus, sehingga tidak terdapat *trade-off* yang perlu dipertimbangkan dalam pemilihan ini.

Model RF Skenario 1 ini selanjutnya menjadi model yang diintegrasikan ke dalam aplikasi web berbasis Flask, sebagaimana dijelaskan pada sub-bab 4.13. Analisis lebih mendalam mengenai komparasi keempat model — termasuk *trade-off* antara Skenario 1 dan Skenario 2, perbedaan performa RF vs SVM, dan interpretasi pola pada *confusion matrix* — disajikan pada Bab V (Pembahasan).

---

## 4.12  Hasil Perancangan Sistem

`[TODO: Sub-bab ini merupakan hasil implementasi dari tahap 3.3.11 Perancangan Sistem pada Bab III. Jelaskan hasil perancangan Use Case Diagram dan Activity Diagram yang telah dibuat.]`

### 4.12.1  Use Case Diagram

`[TODO: Sertakan Gambar Use Case Diagram sistem klasifikasi kepadatan lalu lintas (sudah tersedia di project sebagai 'diagram_usecase.png'). Jelaskan secara singkat setiap use case yang ada:]`

- `[TODO: Login — hak akses Admin, Analis, Staff (DISHUB) dan User (Masyarakat)]`
- `[TODO: Mengupload data file CSV]`
- `[TODO: Memvalidasi dan mengoreksi hasil klasifikasi (extend dari simpan data)]`
- `[TODO: Simpan data]`
- `[TODO: Latih ulang model]`
- `[TODO: Melihat dashboard statistik kepadatan lalu lintas]`
- `[TODO: Melihat prediksi kepadatan lalu lintas]`

### 4.12.2  Activity Diagram

`[TODO: Jelaskan alur aktivitas sistem secara detail dari awal hingga akhir, termasuk decision point dan parallel process yang terjadi. Sertakan gambar Activity Diagram jika sudah dibuat.]`
...
## 4.14  Hasil Evaluasi Sistem (System Usability Scale)

`[TODO: Sub-bab ini merupakan hasil implementasi dari tahap 3.3.13 Evaluasi Sistem pada Bab III. Isi sub-bab ini setelah pengujian SUS selesai dilakukan.]`

Evaluasi sistem dilakukan menggunakan metode *System Usability Scale* (SUS) untuk mengukur tingkat kemudahan dan kepuasan pengguna dalam menggunakan aplikasi web yang telah dikembangkan. SUS merupakan kuesioner standar yang terdiri dari 10 pertanyaan terstruktur dengan skala Likert 1–5, yang dirancang untuk menilai aspek *usability* seperti kemudahan navigasi, konsistensi *interface*, dan efisiensi penggunaan sistem.

Pertanyaan kuesioner SUS yang digunakan disajikan pada Tabel 4.32.

**Tabel 4.32  Pertanyaan Kuesioner System Usability Scale (SUS)**
...
| 7 | Saya merasa kebanyakan orang akan belajar menggunakan sistem ini dengan cepat |
...
Evaluasi dilakukan dengan melibatkan responden yang terdiri dari Admin dan User (Dinas Perhubungan Surabaya) yang telah menggunakan sistem. Hasil kuesioner kemudian dihitung menggunakan formula standar SUS pada Persamaan (2.12):

$$\bar{x} = \frac{\sum x}{n}$$

Keterangan:
- $\bar{x}$ = Rata-rata skor SUS
- $\sum x$ = Total skor SUS dari seluruh responden
- $n$ = Jumlah user/responden

Skor SUS berada pada rentang 0–100, di mana skor di atas 68 dianggap memiliki *usability* yang baik.

> **`[TODO: Setelah pengujian SUS selesai, lengkapi bagian berikut:]`**

> **`[TODO: Tabel 4.33 — Hasil Skor SUS per Responden]`**
> Kolom yang disarankan: `No | Responden | Jabatan | Skor SUS | Kategori`

> **`[TODO: Paragraf interpretasi hasil skor SUS:]`**
> Berdasarkan hasil evaluasi, diperoleh rata-rata skor SUS sebesar X dari Y responden. Mengacu pada interpretasi skor SUS (Bangor et al., 2009), skor tersebut termasuk kategori [Excellent/Good/OK/Poor/Awful]. Skor di atas 68 mengindikasikan bahwa sistem memiliki tingkat usability yang dapat diterima pengguna.

> **`[TODO: Tambahkan kesimpulan singkat di akhir sub-bab 4.14 bahwa sistem dinilai memiliki usability yang baik/cukup baik berdasarkan skor SUS yang diperoleh, dan siap digunakan oleh pengguna di DISHUB Surabaya.]`**

---

## Catatan Penutup Bab IV

Bab IV ini telah memaparkan keseluruhan hasil implementasi penelitian, mulai dari studi literatur, pengumpulan data video CCTV, ekstraksi data menggunakan YOLOv8 + DeepSORT, evaluasi keakuratan ekstraksi dengan MAPE, labeling data dengan acuan PKJI 2023 dan HCM, preprocessing data, pembagian data, modelling dengan Random Forest dan SVM, hyperparameter tuning, evaluasi keempat *best model* pada *testing set*, pemilihan model terbaik (Random Forest Skenario 1), perancangan sistem, pengembangan web Flask, hingga evaluasi sistem menggunakan SUS. Analisis lebih mendalam mengenai hasil yang diperoleh, termasuk interpretasi pola data, pembahasan limitasi sistem, dan perbandingan dengan penelitian terdahulu, akan dilanjutkan pada Bab V.
